...
RSpec.describe Hyrax::Actors::GenericWorkActor do
  ...
  describe '#create' do
    ...
    context 'valid attributes' do
      ...
      before do
        redlock_client_stub
        # The "perform_enqueued" example configuration doesn't work
        # when set in spec_helper, so we'll declare it here instead.
      ...
      after do
        ...
        ActiveJob::Base.queue_adapter.perform_enqueued_jobs = nil
      end
      ...
      context "when setting work-level download permissions (custom code integration)" do
        let(:attributes) do
          attributes_for(:generic_work, admin_set_id: admin_set.id, visibility: visibility).tap do |a|
            a[:permissions_attributes] = [type: "group", name: "public", access: "download"]
          end
        end
        ...
        it "saves the correct attributes" do
          expect(middleware.create(env)).to be true
          curation_concern.reload
          expect(curation_concern.permissions.map(&:to_hash)).to include(type: "group", name: "public", access: "download")
        end
      end
