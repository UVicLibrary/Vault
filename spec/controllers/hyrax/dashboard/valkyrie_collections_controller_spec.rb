...
RSpec.describe Hyrax::Dashboard::CollectionsController, :clean_repo do
  ...
  let(:asset1) { FactoryBot.valkyrie_create(:hyrax_work, title: ["First of the Assets"], edit_users: [user]) }
  let(:asset2) { FactoryBot.valkyrie_create(:hyrax_work, title: ["Second of the Assets"], edit_users: [user]) }
  let(:asset3) { FactoryBot.valkyrie_create(:hyrax_work, title: ["Third of the Assets"], edit_users: [user]) }
  ...
  let(:unowned_asset) { FactoryBot.valkyrie_create(:hyrax_work) }
    ...
    context "with files I can access" do
      it "creates a collection using only the accessible files" do
        ...
        expect(queries.find_members_of(collection: collection.valkyrie_resource).map(&:id))
            .to contain_exactly(asset1.id, asset2.id, unowned_asset.id)
      end
      ...
      it "adds docs to the collection and adds the collection id to the documents in the collection" do
        ...
        expect(assigns[:collection].member_objects.map(&:id)).to contain_exactly(asset1.id, unowned_asset.id)
  ...
  describe "#update" do
    ...
    context 'when moving members between collections' do
      let(:asset1) { FactoryBot.valkyrie_create(:hyrax_work, edit_users: [user]) }
      let(:asset2) { FactoryBot.valkyrie_create(:hyrax_work, edit_users: [user]) }
      let(:asset3) { FactoryBot.valkyrie_create(:hyrax_work, edit_users: [user]) }
        ...
        if collection.is_a? Valkyrie::Resource
          Hyrax::Collections::CollectionMemberService
              .add_members(collection_id: collection.id,
                           ...
                           user: user)
          collection.member_ids = [asset1, asset2, asset3, asset4, asset5].map(&:id)
        else
