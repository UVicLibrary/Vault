    ...
    describe "update collection participants" do
      let(:input_params) do
        { collection_id: permission_template.source_id,
          permission_template: form_attributes,
          referer_anchor: "#discovery" }
      end
      let(:main_app) { Rails.application.routes.url_helpers }
      let(:referer) { "/dashboard/collections/#{permission_template.source_id}/edit?locale=en" }
      ...
      it "is successful" do
        ...
        expect(response).to redirect_to(
                main_app.confirm_collection_access_permission_path(
                  permission_template.source_id,
                  locale: "en",
                  referer: "#{referer}#discovery")
                )
