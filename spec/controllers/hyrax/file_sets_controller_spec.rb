...
RSpec.describe Hyrax::FileSetsController do
    ...
    describe "#update" do
      ...
      context "when updating the attached file already uploaded" do
        ...
        it "spawns a ContentNewVersionEventJob" do
          ...
          expect(assigns[:file_set].modified_date)
            .not_to be file_set.modified_date
          expect(assigns[:file_set].title).to contain_exactly(*file_set.title)
        end
      end
      ...
      it 'denies access to private files' do
        ...
        expect(response)
          .to fail_redirect_and_flash(main_app.new_user_session_path(locale: 'en'),
                                      'You are not authorized to access this page. Log in with your UVic account to access restricted material, or email speccoll@uvic.ca to request access.')
      end
  ...
  describe 'tenant-specific show presenters (#show_presenter)' do
    context 'when in multitenancy mode' do
      context 'and tenant is vault.library.uvic.ca' do
        it 'returns the correct presenter class' do
          expect(controller.show_presenter).to eq VaultFileSetPresenter
        end
      end

      context 'and tenant is iaff.library.uvic.ca' do
        before { allow(account).to receive(:cname).and_return("iaff") }
