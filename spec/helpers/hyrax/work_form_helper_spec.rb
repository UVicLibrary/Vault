...
RSpec.describe Hyrax::WorkFormHelper do
  describe '.form_tabs_for' do
    context 'with a change set style form' do
      ...
      it 'returns a tab list that includes "share"' do
        expect(form_tabs_for(form: form)).to eq ["metadata", "files", "relationships", "share"]
      end
    end
    ...
    context 'with a batch upload form' do
      ...
      it 'returns an alternate tab ordering' do
        expect(form_tabs_for(form: form)).to eq ["files", "metadata", "relationships", "share"]
      end
    end
  end
  ...
  describe '.form_file_set_select_for' do
    ...
    context 'with a ChangeSet-style ResourceForm' do
      ...
      context 'with file_set members', index_adapter: :solr_index, valkyrie_adapter: :test_adapter do
        ...
        let(:file_sets) do
          [FactoryBot.valkyrie_create(:hyrax_file_set, title: 'alphabet.jpg'),
        ...
        it 'gives labels => ids and sorts alphabetically' do
          id1 = file_sets[0].id.to_s
          id2 = file_sets[1].id.to_s
          id3 = file_sets[2].id.to_s

          expect(form_file_set_select_for(parent: form))
              .to eq({ 'alphabet.jpg' => id1,
                       'moominpapa.jpg' => id2,
                       'snorkmaiden.jpg' => id3 })
        end
