RSpec.describe GoogleMapCollectionMembersService do
  ...
  let!(:work1) { create(:generic_work, coordinates: ["52.8577, -2.03686"], title: ["Title A"], member_of_collections: [parent_collection]) }
  let!(:work2) { create(:generic_work, title: ["Title B"], member_of_collections: [parent_collection]) }
  let!(:work3) { create(:generic_work, coordinates: ["53.92527, -3.01085"], title: ["Title C"], member_of_collections: [subcollection]) }
  let!(:work4) { create(:generic_work) }

  describe '#available_member_works' do
    it 'returns only nested works with coordinates' do
