RSpec.describe EdtfDateService do
  ...
  context 'when date is a single date with year-, month-, or day-specificity' do
    ...
    context 'and date is after 1000' do
      ...
      it 'responds to #year_range with the correct years (integers)' do
        expect([year.year_range, month.year_range, day.year_range]).to all(eq([1863]))
      end

      it 'responds to #first_solr_date with a solr-formatted datetime string' do
    ...
    context 'and date is before 1000' do
      ...
      it 'responds to #first_solr_date with a solr-formatted datetime string' do
        expect(year.first_solr_date).to eq("0900-01-01T00:00:00Z")
        expect(month.first_solr_date).to eq("0900-03-01T00:00:00Z")
        expect(day.first_solr_date).to eq("0900-06-01T00:00:00Z")
      end
    end
  ...
  context 'when date is a season' do
    ...
    it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
  ...
  context 'when date is unspecified, a century, or a decade' do
    ...
    context 'and is after 1000' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect(century.first_solr_date).to eq "1900-01-01T00:00:00Z"
        expect(decade.first_solr_date).to eq "1920-01-01T00:00:00Z"
      end
    end
    ...
    context 'and date is before 1000' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect(century.first_solr_date).to eq "0900-01-01T00:00:00Z"
        expect(decade.first_solr_date).to eq "0970-01-01T00:00:00Z"
      end
  ...
  context 'when date is an interval' do
    ...
    context 'that is open-ended' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect(open_start.first_solr_date).to eq(open_end.first_solr_date) & eq("1900-01-01T00:00:00Z")
        expect(pre_1000_start.first_solr_date).to eq "0950-01-01T00:00:00Z"
        expect(pre_1000_end.first_solr_date).to eq "0800-01-01T00:00:00Z"
      end
    ...
    context 'of dates, both after 1000 AD' do
      ...
      it 'responds to #year_range with an years (integers)' do
        expect(service.year_range).to eq([1989, 1990])
      end

      it 'responds to #first_solr_date with an solr-formatted datetime string' do
        expect(service.first_solr_date).to eq "1989-11-09T00:00:00Z"
      end
    ...
    context 'of seasons' do
      ...
      it 'responds to #first_solr_date with an solr-formatted datetime string' do
        expect(service.first_solr_date).to eq "2011-06-01T00:00:00Z"
      end
    ...
    context 'of approximate (~), uncertain (?), or approximate and uncertain (%) dates' do
      ...
      it 'responds to #first_solr_date with an solr-formatted datetime string' do
        expect(approx.first_solr_date).to eq(approx_and_uncertain.first_solr_date) & eq("1965-10-01T00:00:00Z")
        expect(uncertain.first_solr_date).to eq "1965-10-01T00:00:00Z"
      end
    ...
    context 'of centuries or decades' do
      ...
      it 'responds to #first_solr_date with an solr-formatted datetime string' do
        expect(century.first_solr_date).to eq "1400-01-01T00:00:00Z"
        expect(decade.first_solr_date).to eq "1980-01-01T00:00:00Z"
      end
    ...
    context 'including date(s) before 1000' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect(interval.first_solr_date).to eq "0950-01-01T00:00:00Z"
        expect(interval_before_1000.first_solr_date).to eq "0950-01-01T00:00:00Z"
      end
  ...
  context 'when date is questionable and/or approximate' do
    ...
    context 'and has year-specificity' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect([approx_service.first_solr_date, uncertain_service.first_solr_date, approx_and_uncertain_service.first_solr_date]).to all(eq("1965-01-01T00:00:00Z"))
      end
    end
    ...
    context 'and has month-specificity' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect([approx_service.first_solr_date, uncertain_service.first_solr_date, approx_and_uncertain_service.first_solr_date]).to all(eq("1975-10-01T00:00:00Z"))
      end
    end
    ...
    context 'and has day-specificity' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect([approx_service.first_solr_date, uncertain_service.first_solr_date, approx_and_uncertain_service.first_solr_date]).to all(eq("1985-10-05T00:00:00Z"))
      end
    end
    ...
    context 'and is before 1000' do
      ...
      it 'responds to #first_solr_date with the correct solr-formatted datetime string' do
        expect([approx_service.first_solr_date, uncertain_service.first_solr_date, approx_and_uncertain_service.first_solr_date]).to all(eq("0905-06-01T00:00:00Z"))
      end
  ...
  context 'when date is "unknown" or "no date"' do
    let(:unknown) { EdtfDateService.new("unknown") }
    let(:no_date) { EdtfDateService.new("no date") }

    it 'responds to #humanize with "unknown" or "no date"' do
      expect(unknown.humanized).to eq "unknown"
      expect(no_date.humanized).to eq "no date"
    end

    it 'responds to #year_range with nil' do
      expect(unknown.year_range).to be_nil
      expect(no_date.year_range).to be_nil
    end

    it 'responds to #first_solr_date with nil' do
      expect(unknown.first_solr_date).to be_nil
      expect(no_date.first_solr_date).to be_nil
    end
