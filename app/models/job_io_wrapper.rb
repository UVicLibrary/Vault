...
class JobIoWrapper < ApplicationRecord
  ...
  def self.create_with_varied_file_handling!(user:, file:, relation:, file_set:)
    ...
    if file.is_a?(Hyrax::UploadedFile)
      ...
      # if file.path.start_with?("/tmp/RackMultipart") # When updating a file set with new content,
      #   # Hyrax doesn't recognize an uploaded file as uploaded and tries to use this path instead.
      #   file = Hyrax::UploadedFile.create(user: user, file: file)
      #   args[:uploaded_file] = file
      #   args[:path] = file.uploader.path
      # else
        args[:path] = file.path
  ...
  def ingest_file
    Rails.logger.debug "Hit Job IO Wrapper. Ingesting file..."
