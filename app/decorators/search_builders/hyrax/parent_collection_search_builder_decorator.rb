...
Hyrax::ParentCollectionSearchBuilder.class_eval do
  ...
  def include_item_ids(solr_parameters)
    solr_parameters[:fq] ||= []
    solr_parameters[:fq] += ["has_model_ssim:#{Hyrax.config.collection_class}"]
    solr_parameters[:fq] += [Hyrax::SolrQueryService.new.with_ids(ids: item.member_of_collection_ids).build] if item.member_of_collection_ids.any?
  end
