class BatchExportJob < ActiveJob::Base
  ...
  # disaster that wipes out local backups.
  #
  # Bag structure:
  #   Top-level dir named after file set id
  #     |__ data
  #           |__objects
  #                 |__filename.ext - latest version of a file (downloaded from Fedora)
  #                 |__bitstream_file_set_id
  #           |__metadata.txt - file metadata e.g. mime type, duration, date of last fixity check
  #     |__bag-info.txt - generated automatically by bagit gem
  ...
  #                               (files automatically generated by bagit gem)
    ...
    Dir.chdir(dirname) do
      works.each do |work|
        work.file_sets.each do |file_set|
          ...
          bag_dir = "#{file_set.id}"
          next if File.file? "#{bag_dir}.7z" or File.file? "/mnt/narwhal/#{bag_dir}.7z"
  ...
  def download_objects(file_set)
    ...
    ext = File.extname(File.basename(file_set.import_url))
  ...
  def skip_thumbnail?(file_set)
    # If file set is not the parent's representative AND
