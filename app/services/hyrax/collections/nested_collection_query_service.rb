  ...
  module Collections
    module NestedCollectionQueryService
      ...
      def self.parent_and_child_can_nest?(parent:, child:, scope:)
        ...
        return false unless same_collection_type?(parent, child)
      ...
      # After upgrading Hyrax to 3.0.2, newly-created collections have a gid like
      # "gid://hyku/Hyrax::CollectionType/2" whereas old collections with what SHOULD be
      # the same gid have a gid like "gid://hyku/hyrax-collectiontype/2". This is a
      # (hopefully) temporary patch until we migrate all collections to one type or the other.
      def self.same_collection_type?(parent, child)
        parent_collection_type = parent.collection_type_gid.gsub('hyrax-collectiontype','').gsub('Hyrax::CollectionType','')
        child_collection_type = child.collection_type_gid.gsub('hyrax-collectiontype','').gsub('Hyrax::CollectionType','')
        parent_collection_type == child_collection_type
      end
